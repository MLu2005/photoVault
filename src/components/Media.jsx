...
export function MediaCard({ item, selected, selectMode, onSelect, onOpen, onFavorite, busy }) {
  return <article className={`media-card ${selected ? 'selected' : ''}`}>
    <button className="media-open" onClick={selectMode ? onSelect : onOpen} aria-label={`${selectMode ? 'Zaznacz' : 'Otwórz'} ${item.name}`}><Preview item={item}/></button>
    <label className={`media-select ${selectMode ? 'visible' : ''}`}><input type="checkbox" checked={selected} onChange={onSelect} aria-label={`Zaznacz ${item.name}`} disabled={busy}/><span><Icon name="check" size={14}/></span></label>
    {!item.trashedAt && <button className={`favorite-button ${item.favorite ? 'is-favorite' : ''}`} disabled={busy} onClick={onFavorite} aria-label={`${item.favorite ? 'Usuń z ulubionych' : 'Dodaj do ulubionych'}: ${item.name}`} aria-pressed={item.favorite}><Icon name="heart" size={16} filled={item.favorite}/></button>}
...
export function MediaRow({ item, selected, onSelect, onOpen, onFavorite, busy }) {
  return <div className={`media-row ${selected ? 'selected' : ''}`}>
    <input type="checkbox" checked={selected} onChange={onSelect} aria-label={`Zaznacz ${item.name}`} disabled={busy}/>
    ...
    {!item.trashedAt && <button className={`icon-button ${item.favorite ? 'is-favorite' : ''}`} onClick={onFavorite} disabled={busy} aria-label={`Ulubione: ${item.name}`} aria-pressed={item.favorite}><Icon name="heart" size={18} filled={item.favorite}/></button>}
  </div>;
}
