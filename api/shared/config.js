...
function authConfig() {
  ...
  if (secret.length < 32) fail(503, 'Skonfiguruj PV_SESSION_SECRET i PV_USERS w ustawieniach API.', 'CONFIGURATION');
  let users;
  ...
  if (!Array.isArray(users) || !users.length || users.length > 10 || users.some(u =>
    ...
    fail(503, 'Brak poprawnej konfiguracji kont PV_USERS. Uruchom npm run setup.', 'CONFIGURATION');
  }
  ...
  try {
    ...
    origin = url.origin;
  } catch { fail(503, 'PV_APP_ORIGIN musi zawierać dokładny adres aplikacji, bez końcowego /.', 'CONFIGURATION'); }
