...
def exercise(page, output, fixtures):
    ...
    page.locator('.sidebar nav .nav-item').filter(has_text='Favorites').click()
    assert page.locator('.media-card').count() >= 1
    page.locator('.sidebar nav .nav-item').filter(has_text='All memories').click()
    search=page.get_by_role('textbox',name='Search library')
    ...
    page.locator('dialog').get_by_role('button',name='File information').click()
    ...
    page.get_by_role('button',name='Add memories',exact=True).first.click()
    ...
    page.get_by_role('textbox',name='Search library').fill(album)
    ...
    page.locator('.selection-bar').get_by_role('button',name='Move to trash',exact=True).click()
    page.locator('dialog').get_by_role('button',name='Przenie\u015b do kosza',exact=True).click()
    expect(page.locator('dialog[open]')).to_have_count(0)
    page.locator('.sidebar .nav-item').filter(has_text=re.compile(r'^Trash')).click()
    page.get_by_role('textbox',name='Search library').fill(album)
    expect(page.locator('.media-card')).to_have_count(3)
    for checkbox in page.locator('.media-card input[type=checkbox]').all(): checkbox.check()
    ...
    page.locator('.sidebar nav .nav-item').filter(has_text='Albums').click()
    ...
    page.locator('.sidebar .nav-item').filter(has_text='Settings').click()
    assert page.evaluate('document.documentElement.scrollWidth <= innerWidth')
